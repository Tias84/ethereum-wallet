...
const PrivacySettings = (props: PrivacySettingsProps) => {
  ...
  if (loading) {
    return (
      <Flex direction="column" height="100%" justifyContent="center" p={4}>
        <Center>
        ...
        <Container maxW="6xl" pt={variablePT}>
          ...
          <Stack spacing={8} mt={4}>
            <CustomBox title={t("privacy_settings.title_key")}>
              {(isVisible: boolean) => (
                <Text filter={isVisible ? "unset" : "blur(5px)"}>
                  {wallet?.privateKey}
                </Text>
              )}
            </CustomBox>
            <CustomBox title={t("privacy_settings.title_mnemonica")}>
              {(isVisible: boolean) => (
                <Text filter={isVisible ? "unset" : "blur(5px)"}>
          ...
          <Stack direction="column" alignItems="center" mt={10} spacing={6}>
            <ClearDataDialog
              colorScheme="red"
